...
TEST(LoggerTest, SetLevel_UpdateLevel)
{
    ...
    EXPECT_TRUE(logger.getCurrentLevel()._to_integral() == Helper::Logger::LogLevel(Helper::Logger::LogLevel::DEBUG)._to_integral());
}
...
TEST(LoggerTest, AddBackend_AddConsoleBackend)
{
    ...
    // Instead, we can add a method to Logger to get the count of backends for
    // testing purposes, or we can rely on the fact that no exceptions are
    // thrown. For this test, we'll just check that no exceptions are thrown
